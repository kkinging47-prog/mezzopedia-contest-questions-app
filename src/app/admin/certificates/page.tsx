...
export default function AdminCertificatesPage() {
  ...
  useEffect(() => { fetch('/api/admin/me').then(res => { if (!res.ok) throw new Error('Admin login required.'); return loadAll(); }).catch(err => { setError(err.message || 'Could not open certificates page.'); setLoading(false); }); }, []);
  ...
  async function importEmails() {
    const rows = emailCsv.split(/\r?\n/).map(line => line.trim()).filter(Boolean).map(line => {
      const [usercode, email] = line.split(',').map(value => value.trim());
      return { usercode, email };
    });
    if (!rows.length) { setError('Paste email rows first.'); return; }
    setMessage('Saving participant emails...'); setError('');
    const res = await fetch('/api/admin/certificates/emails', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rows }) });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) { setError(json.error || 'Could not save emails.'); setMessage(''); return; }
    setEmailCsv('');
    setMessage(`Email import complete. Updated ${json.updated || 0}. Failed: ${(json.failed || []).length}. ${(json.failed || []).join('; ')}`);
    loadAll();
  }
...
function NumberField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label><span className="label">{label}</span><input className="input" type="number" step="1" value={value} onChange={e => onChange(Number(e.target.value || 0))} /></label>; }
