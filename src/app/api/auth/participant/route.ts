...
function questionLimitFor(settings: QuestionCountSettings, stage: string, category: string) {
  const normalizedStage = normalizeContestStage(stage);
  const raw = Number(settings?.[normalizedStage]?.[category]);
...
async function pickUniqueQuestionOrder(questionIds: string[], requestedCount: number, category: string, stage: string) {
  ...
  const { data: previous } = await supabaseAdmin
    ...
    .eq('contest_stage', normalizedStage)
...
export async function POST(request: Request) {
  ...
  if (enteredName && enteredName.toLowerCase() !== String(participant.name || '').toLowerCase()) {
    await supabaseAdmin.from('admin_audit_logs').insert({ action: 'NAME_MISMATCH_ON_SIGNIN', entity_type: 'participant', entity_id: participant.id, details: { enteredName, registeredName: participant.name } });
  }
